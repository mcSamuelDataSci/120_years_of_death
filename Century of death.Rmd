...
-Data from the CCB was compared to data obtained for the same years from CDC WONDER in order to verify proper sorting/classification of death causes.
-These spreadsheets were then inputted into R to allow for statistical analysis and the development of visual aides.
<br>
...
# Figure 1. Yearly crude death rate per 100,000 is stratified by topLevel with the purpose of revealing long-term mortality trends
...
# Figure 2. Change in specificLevel mortality between 1900 and 2020 grouped by topLevel. Each year has an x-axis of varying magnitude, depending on the number of deaths in that year.
...
ggplot(workGroup1a, aes(x=fct_rev(specificLevel), y=deaths, fill = topLevel)) +
  ...
  geom_col() +
  coord_flip() +
  labs(title = "2) Number of Deaths as Sorted by specificLevel",
...
ggplot(workGroup1a, aes(x=fct_rev(displayLevel), y=cDeathRate, fill = topLevel)) +
  ...
  geom_col() +
  coord_flip() +
  labs(title = "3) Crude Death Rates (displayLevel)",
       y = "Crude Death Rate per 100k Pop", x = "Cause of Death") +
